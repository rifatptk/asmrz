...
export default function Sidenav2() {
  ...
  return (
    <div className="bg-gray-800 p-10">
      <h1 className="tracking-widest mb-5">SIDENAV TWO</h1>
      ...
      <div className="bg-gray-900 rounded-lg overflow-hidden">
        ...
        <div className="bg-gray-500 px-4 py-2 flex items-center gap-2 shadow-xl">
          ...
          >
            {isSidebarShown ? <IoIcons.IoMdClose /> : <FiMenu />}
          </button>
        ...
        <div className="flex">
          <aside
            style={{ width: isSidebarShown ? sidebarWidth : 0 }}
            className={`${
              isSidebarShown ? `w-[${sidebarWidth}px] px-2` : "w-0"
...
function NavItem({ icon, path, title, subNav }) {
  ...
  return subNav ? (
    <div className="px-4 py-1 cursor-pointer">
      ...
      {isExpanded &&
        subNav.map((link) => <NavItem key={link.path} {...link} />)}
    </div>
  ) : (
